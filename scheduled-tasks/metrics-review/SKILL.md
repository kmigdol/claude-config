...
3. **Read the metric from wherever it actually lives.** Most Monitoring tickets here are pipeline/infra work, and their observables are **not** PostHog events. Pick the source from the ticket:
   ...
   - **Supabase org usage page — did NOT render under automation on 2026-08-24.** `https://supabase.com/dashboard/org/<org>/usage` loaded authenticated (title resolved to "Supabase", not a sign-in page) but `document.documentElement.innerText.length` was **0** across two reads six seconds apart, and `get_page_text` returned "No text content found". A guessed `/api/platform/organizations/<org>/daily-stats` endpoint 404s. This page HAS been read this way before, so treat it as flaky rather than closed: **attempt it, and if the document is empty, report the symptom and say the number is unread** — Kayleigh can read it in a normal browser tab in seconds. Never substitute a proxy for monthly egress.
